...
class logger_impl {
 public:
  ...
  logger_impl(std::shared_ptr<service_impl> service,
              ...
              base::vector<service::sink_ptr>& sinks, const bool async)
      : service_(service),
        name_(name),
 ...
 private:
  ...
  base::string name_;
  /** Sink集合 */
  base::vector<service::sink_ptr> sinks_;
  /** 日志级别（使用原子操作以支持线程安全） */
...
logger::logger(ctor_key, service& service, const std::string_view& name,
               base::vector<sink_ptr> sinks, bool async)
    : impl_(base::make_unique<logger_impl>(service.get_impl(), name, sinks,
                                             async)) {}
...
void logger::log(std::uint32_t sid, level lv, base::buffer_1k& buf,  // NOLINT
