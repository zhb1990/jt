...
#ifdef _WIN32
#include "../../../detail/platform/win32.h"
#else
#include <unistd.h>
...
class sink_console_impl {
 public:
   ...
   */
  void write(const level lv, const base::buffer_1k& buf,
 ...
 private:
   ...
   * @param end 结束位置
   */
  void write_range(const base::buffer_1k& buf, const std::size_t start,
 ...
 * @param buf 要输出的数据缓冲区
 */
void write_stdout(const base::buffer_1k& buf) {
 ...
 * @param buf 要输出的数据缓冲区
 */
void write_stderr(const base::buffer_1k& buf) {
