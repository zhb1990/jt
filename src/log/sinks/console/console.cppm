...
class JT_API sink_stdout final : public sink {
 ...
 protected:
   ...
   */
  void write(level lv, const time_point&, const base::buffer_1k& buf,
 ...
 * @param buf 要输出的数据缓冲区
 */
JT_API void write_stdout(const base::buffer_1k& buf);

/**
 * 写入数据到标准错误（错误级别）
 * @param buf 要输出的数据缓冲区
 */
JT_API void write_stderr(const base::buffer_1k& buf);
