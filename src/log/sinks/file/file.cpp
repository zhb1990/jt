...
class sink_file_imp {
 ...
 private:
  ...
  base::string name_{};
  // 日志文件目录
  base::string directory_{};
  // LZ4压缩文件目录
  base::string lz4_directory_{};
  // 单个日志文件最大大小
...
sink_file::sink_file(service& s, const sink_file_config& config)
    : impl_(base::make_unique<sink_file_imp>(s.make_lz4_client(), config)) {}
...
void sink_file::write(level, const time_point& point,
                      const base::buffer_1k& buf, std::size_t, std::size_t) {
