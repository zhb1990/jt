...
int main(int argc, char** argv) {
  // 打印初始内存使用情况
  std::println("mem {}", jt::base::allocated_memory());
  {
    // 配置文件日志Sink
    ...
    std::array a1{
        jt::base::make_dynamic_unique<jt::log::sink, jt::log::sink_file>(
            service, config),
        jt::base::make_dynamic_unique<jt::log::sink, jt::log::sink_stdout>()};
    // 创建异步日志记录器
    const auto log_ptr = service.create_logger(std::move(a1), "test", true);
    auto& log = *log_ptr;
    jt::log::info(log, "mem {}", jt::base::allocated_memory());

    // 测试固定大小缓冲区
    jt::base::base_memory_buffer<1> b1;
    b1.shrink();  // 收缩缓冲区
    jt::log::info(log, "readable {}", b1.readable());

    {
      // 测试缓冲区操作
      jt::base::base_memory_buffer<1> buffer;
      ...
      jt::log::info(log, "mem {}", jt::base::allocated_memory());
      const jt::base::read_buffer rb(buffer);  // 转换为只读缓冲区
    ...
    {
      // 测试原始内存分配
      void* ptr = jt::base::allocate(100);
      jt::log::info(log, "mem {}", jt::base::allocated_memory());
      jt::log::info(log, "ptr size {}", jt::base::allocated_size(ptr));
      jt::base::deallocate(ptr);  // 释放内存
      jt::log::info(log, "mem {}", jt::base::allocated_memory());
    }

    // 使用变量参数记录警告日志（中文）
    const std::string fmt = "使用的内存 {}";
    jt::log::vwarn(log, fmt, jt::base::allocated_memory());
    std::println("mem {}", jt::base::allocated_memory());
  }
  std::println("mem {}", jt::base::allocated_memory());
